...
// A resolved path is servable only if it is exactly one of the allow-listed
// real paths (for file entries like "index.html") or lives inside one of
// them (for directory entries like "assets"). This stops a symlink placed
// inside an allow-listed directory (e.g. assets/leak.js -> ../.git/config)
// from being served just because it is nested under the repo root.
function isInsideAllowRoots(realAllowRoots, realPath) {
  for (const allowRoot of realAllowRoots) {
    if (realPath === allowRoot || realPath.startsWith(allowRoot + path.sep)) return true;
  }
  return false;
}
...
function createStaticServer(rootDir, options) {
  ...
  const realAllowRoots = [];
  for (const entry of allow) {
    try {
      realAllowRoots.push(fs.realpathSync(path.join(realRoot, entry)));
    } catch (err) {
      // Allow-listed entry does not exist on disk; skip it.
    }
  }
  ...
  return http.createServer((req, res) => {
    ...
    fs.realpath(target, (realErr, real) => {
      if (realErr || !isInside(realRoot, real) || !isInsideAllowRoots(realAllowRoots, real)) { plain(res, 404, 'Not Found'); return; }
